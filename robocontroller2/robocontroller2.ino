...
//init variables
int targetpos[] = {0, 100, 100};  //XPOS IN STEPS, HEAD ANGLE, HEIGHT ACTUATOR
int currentpos[] = {0, 100, 100}; //STORE THE CURRENT POSITION HERE AFTER MOTOR IS NO LONGER BUSY
int deltapos[] = {0, 0, 0};
int pos = 0;

void setup() {
  //attach servos to ports
  HeadAngle.attach(6, 10, 235);  // attaches the servo on pin 10 to the servo object
  Height.attach(5, 10, 235);
...
void loop() {
  ...
  else
  {
    ...
    {
      Serial.println("STEPPER BUSY! Try again later");
    }
  }
  //  Serial.flush();
  //
